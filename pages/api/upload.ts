...
function getSingleFieldValue(v: string | string[] | undefined): string | undefined {
  if (Array.isArray(v)) return v[0]
  return v
}

function getSingleFile(file: FormidableFile | FormidableFile[] | undefined): FormidableFile | undefined {
...
export default async function handler(req: NextApiRequest, res: NextApiResponse<UploadResponse>) {
  ...
  try {
    ...
    const kindRaw = getSingleFieldValue(fields.kind as string | string[] | undefined)

    if (typeof kindRaw !== 'string' || !ALLOWED_KINDS.has(kindRaw)) {
      return res
        .status(400)
        .json({ error: 'Invalid or missing kind (avatar | background | cv | project)' })
    }

    const file = getSingleFile(files.file as FormidableFile | FormidableFile[] | undefined)
    if (!file) {
    ...
    } else if (kindRaw === 'background') {
      folder = 'portfolio/background'
    } else if (kindRaw === 'cv') {
      folder = 'portfolio/cv'
    } else {
      const idxRaw = getSingleFieldValue(fields.projectIndex as string | string[] | undefined)
      const n = typeof idxRaw === 'string' && /^\d+$/.test(idxRaw) ? Number(idxRaw) : 0
      folder = `portfolio/projects/${n}`
    }
