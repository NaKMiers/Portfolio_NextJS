...
// .agents/skills/browse/src/buffers.ts
...
// .agents/skills/browse/src/url-validation.ts
...
// .agents/skills/browse/src/config.ts
var exports_config = {};
...
// .agents/skills/browse/src/platform.ts
...
// .agents/skills/browse/src/cdp-inspector.ts
...
async function inspectElement(page, selector, options) {
  ...
  try {
    ...
    throw new Error(`Element not found: ${selector} â€” ${err.message}`);
  }
...
// .agents/skills/browse/src/read-commands.ts
...
${html}` : html;
    ...
    case "links": {
      ...
      return links.map((l) => `${l.text} â†’ ${l.href}`).join(`
`);
    }
    case "forms": {
...
`);
    ...
    case "network": {
      ...
      return networkBuffer.toArray().map((e) => `${e.method} ${e.url} â†’ ${e.status || "pending"} (${e.duration || "?"}ms, ${e.size || "?"}B)`).join(`
`);
    }
    case "dialog": {
      if (args[0] === "--clear") {
      ...
      return dialogBuffer.toArray().map((e) => `[${new Date(e.timestamp).toISOString()}] [${e.type}] "${e.message}" â†’ ${e.action}${e.response ? ` "${e.response}"` : ""}`).join(`
`);
    }
    case "is": {
      ...
      for (const storeType of ["localStorage", "sessionStorage"]) {
        ...
        for (const [key, value] of Object.entries(store)) {
          ...
          if (SENSITIVE_KEY.test(key) || SENSITIVE_VALUE.test(value)) {
            store[key] = `[REDACTED â€” ${value.length} chars]`;
          }
        }
      }
...
`);
    ...
    case "inspect": {
      ...
      if (showHistory) {
        ...
        return history.map((m, i) => `[${i}] ${m.selector} { ${m.property}: ${m.oldValue} â†’ ${m.newValue} } (${m.source}, ${m.method})`).join(`
`);
      }
      if (!selector) {
        ...
        if (stored) {
          ...
          if (stale)
            output = `âš  Data may be stale (>60s old)
...
// .agents/skills/browse/src/cookie-import-browser.ts
...
// .agents/skills/browse/src/write-commands.ts
...
async function handleWriteCommand(command, args, bm) {
  ...
  switch (command) {
    ...
    case "click": {
      ...
      if (role === "option") {
        ...
        if ("locator" in resolved2) {
          ...
          if (optionInfo) {
            ...
            return `Selected "${optionInfo.text}" (auto-routed from click on <option>) â†’ now at ${page.url()}`;
          }
        }
      }
      const resolved = await bm.resolveRef(selector);
      ...
      return `Clicked ${selector} â†’ now at ${page.url()}`;
    }
    case "fill": {
...
Select domains to import, then close the picker when done.`;
    ...
    case "style": {
      ...
      return `Style modified: ${selector} { ${property}: ${mod.oldValue || "(none)"} â†’ ${value} } (${mod.method})`;
    }
    case "cleanup": {
...
// .agents/skills/browse/src/browser-manager.ts
init_buffers();
init_url_validation();
import {
  chromium
} from "playwright";
var __dirname = "C:\\ME\\IT_IT\\Webs\\Port4lio\\.agents\\skills\\browse\\src";
...
class BrowserManager {
  ...
  async launchHeaded(authToken) {
    ...
    if (extensionPath) {
      ...
      if (authToken) {
        ...
        try {
          fs3.writeFileSync(authFile, JSON.stringify({ token: authToken }), {
            mode: 384
          });
        } catch (err) {
  ...
  async resolveRef(selector) {
    if (selector.startsWith("@e") || selector.startsWith("@c")) {
      ...
      if (count === 0) {
        throw new Error(`Ref ${selector} (${entry.role} "${entry.name}") is stale â€” element no longer exists. ` + `Run 'snapshot' for fresh refs.`);
      }
  ...
  async handoff(message) {
    ...
    try {
      ...
      if (extensionPath) {
        ...
        console.log("[browse] Handoff: extension not found â€” headed mode without side panel");
      }
      const userDataDir = path2.join(process.env.HOME || "/tmp", ".gstack", "chromium-profile");
      fs2.mkdirSync(userDataDir, { recursive: true });
      ...
      return `ERROR: Cannot open headed browser â€” ${msg}. Headless browser still running.`;
    }
    try {
...
`);
      ...
      const msg = err instanceof Error ? err.message : String(err);
      return `ERROR: Handoff failed during state restore â€” ${msg}. Headless browser still running.`;
    }
  }
  resume() {
    this.clearRefs();
...
// .agents/skills/browse/src/server.ts
init_read_commands();
init_write_commands();

// .agents/skills/browse/src/snapshot.ts
init_platform();
...
var SNAPSHOT_FLAGS = [
  ...
  { short: "-C", long: "--cursor-interactive", description: "Cursor-interactive elements (@c refs â€” divs with pointer, onclick)", optionKey: "cursorInteractive" }
];
  ...
  if (opts.cursorInteractive) {
    try {
      ...
      if (cursorElements.length > 0) {
        output.push("");
        output.push("â”€â”€ cursor-interactive (not in ARIA tree) â”€â”€");
        let cRefCounter = 1;
      ...
      output.push("");
      output.push("(cursor scan failed â€” CSP restriction)");
    }
  }
  bm.setRefMap(refMap);
  if (output.length === 0) {
...
(no previous snapshot to diff against â€” this snapshot stored as baseline)`;
    }
...
// .agents/skills/browse/src/meta-commands.ts
init_read_commands();

// .agents/skills/browse/src/commands.ts
...
function wrapUntrustedContent(result, url) {
  ...
  const safeResult = result.replace(/--- (BEGIN|END) UNTRUSTED EXTERNAL CONTENT/g, "--- $1 UNTRUSTED EXTERNAL Câ€‹ONTENT");
...
var COMMAND_DESCRIPTIONS = {
  ...
  links: { category: "Reading", description: 'All links as "text â†’ href"' },
  ...
  press: { category: "Interaction", description: "Press key â€” Enter, Tab, Escape, ArrowUp/Down/Left/Right, Backspace, Delete, Home, End, PageUp, PageDown, or modifiers like Shift+Enter", usage: "press <key>" },
  ...
  watch: { category: "Meta", description: "Passive observation â€” periodic snapshots while user browses", usage: "watch [stop]" },
  ...
  inspect: { category: "Inspection", description: "Deep CSS inspection via CDP â€” full rule cascade, box model, computed styles", usage: "inspect [selector] [--all] [--history]" },
...
async function handleMetaCommand(command, args, bm, shutdown) {
  switch (command) {
    case "tabs": {
      ...
      return tabs.map((t) => `${t.active ? "â†’ " : "  "}[${t.id}] ${t.title || "(untitled)"} â€” ${t.url}`).join(`
`);
    }
    case "tab": {
...
`);
    ...
    case "screenshot": {
      ...
      for (let i = 0;i < args.length; i++) {
        if (args[i] === "--viewport") {
          ...
          if (parts.length !== 4 || parts.some(isNaN))
            throw new Error("Usage: screenshot --clip x,y,width,height â€” all must be numbers");
      ...
      if (clipRect && targetSelector) {
        throw new Error("Cannot use --clip with a selector/ref â€” choose one");
      }
      if (viewportOnly && clipRect) {
        throw new Error("Cannot use --viewport with --clip â€” choose one");
      }
      if (targetSelector) {
...
${wrapUntrustedContent(snapshot, bm.getCurrentUrl())}`;
    ...
    case "disconnect": {
      if (bm.getConnectionMode() !== "headed") {
        return "Not in headed mode â€” nothing to disconnect.";
      }
...
`);
      ...
      bm.startWatch();
      return "WATCHING â€” observing user browsing. Periodic snapshots every 5s.\nRun `$B watch stop` to stop and get summary.";
    }
    case "inbox": {
      const { execSync } = await import("child_process");
      let gitRoot;
      try {
        ...
        return "Not in a git repository â€” cannot locate inbox.";
      }
...
âš ï¸  Cookies stored in plaintext. Delete when no longer needed.`;
      }
      if (action === "load") {
...
// .agents/skills/browse/src/cookie-picker-routes.ts
init_cookie_import_browser();

// .agents/skills/browse/src/cookie-picker-ui.ts
...
<title>Cookie Import â€” gstack browse</title>
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  /* â”€â”€â”€ Browser Pills â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
  .browser-pills {
    display: flex;
    gap: 8px;
    padding: 0 20px 12px;
  ...
  /* â”€â”€â”€ Profile Pills â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
  .profile-pills {
    display: flex;
    gap: 6px;
    padding: 0 20px 12px;
  ...
  /* â”€â”€â”€ Domain List â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
  .domain-list {
    flex: 1;
    overflow-y: auto;
    padding: 0 12px;
  ...
  /* â”€â”€â”€ Imported Panel â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
  .imported-empty {
    flex: 1;
    display: flex;
    align-items: center;
  ...
  /* â”€â”€â”€ Spinner â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
  .spinner {
...
(function() {
  ...
  let allProfiles = [];
  let allDomains = [];
  let importedSet = {};  // domain â†’ count
  let inflight = {};     // domain â†’ true (prevents double-click)
  ...
  function showBanner(msg, type, retryFn) {
    ...
    html += '<button class="banner-close" id="banner-close">Ã—</button>';
  ...
  // â”€â”€â”€ Select Browser â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  // â”€â”€â”€ Render Profile Pills â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  // â”€â”€â”€ Select Profile â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  // â”€â”€â”€ Load Domains â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  // â”€â”€â”€ Render Source Domains â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  function renderSourceDomains() {
    ...
    $sourceFooter.textContent = totalDomains + ' domains Â· ' + totalCookies.toLocaleString() + ' cookies';
  ...
  // â”€â”€â”€ Import Domain â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  // â”€â”€â”€ Import All â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  // â”€â”€â”€ Render Imported â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  function renderImported() {
    ...
    $importedFooter.textContent = entries.length + ' domains Â· ' + totalCookies.toLocaleString() + ' cookies imported';
  ...
  // â”€â”€â”€ Remove Domain â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// .agents/skills/browse/src/sidebar-utils.ts
...
// .agents/skills/browse/src/activity.ts
init_buffers();
...
function generateHelpText() {
  ...
  const lines = ["gstack browse â€” headless browser for AI agents", "", "Commands:"];
...
function loadSession() {
  try {
    ...
    if (session.worktreePath && !fs6.existsSync(session.worktreePath)) {
      console.log(`[browse] Stale worktree path: ${session.worktreePath} â€” clearing`);
...
`;
    ...
    if (newNetworkCount > 0) {
      ...
      const lines = entries.map((e) => `[${new Date(e.timestamp).toISOString()}] ${e.method} ${e.url} â†’ ${e.status || "pending"} (${e.duration || "?"}ms, ${e.size || "?"}B)`).join(`
`) + `
`;
    ...
    if (newDialogCount > 0) {
      ...
      const lines = entries.map((e) => `[${new Date(e.timestamp).toISOString()}] [${e.type}] "${e.message}" â†’ ${e.action}${e.response ? ` "${e.response}"` : ""}`).join(`
`) + `
`;
